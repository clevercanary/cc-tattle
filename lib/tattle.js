 ...
 *
 * @param config {Object}
 * @param emailKeys {Array}
 */
Tattle.prototype.initialize = function(config, emailKeys) {

    this._config = config;

    if (emailKeys && Array.isArray(emailKeys)) {
        this._emailKeys = emailKeys;
    }
    else {
        this._emailKeys = ["_id"];
    }
...
Tattle.prototype.expressErrorHandler = function(app) {
    ...
    app.post("/api/errorlogs", function(req, res, next) {
        ...
        errorLogService.createErrorLog(errorLog, function(dbError, savedErrorLog) {
            ...
            self._emailError(null, savedErrorLog, ccExpressUtils.setupResponseCallback(res));
        });
    });
    ...
    app.use(function(error, req, res, next) {
        ...
        errorLogService.createErrorLog(errLog, function(dbError, savedErrorLog) {
            ...
            self._emailError(error, savedErrorLog, ccExpressUtils.setupResponseCallback(res));
        });
    });
};
...
Tattle.prototype._handleUncaughtException = function() {
    ...
    process.on("uncaughtException", function(error) {
        ...
        errorLogService.createErrorLog(errorLog, function(dbError, savedErrorLog) {
            ...
            process.exitCode = 1;
            self._emailError(error, savedErrorLog, process.exit);
        });
    });
};
 ...
 *
 * @param error {Error}
 * @param savedErrorLog {Object}
 * @param next {Function}
 * @private
 */
Tattle.prototype._emailError = function(error, savedErrorLog, next) {
    ...
    var emailOptions = this._buildEmailOptions(savedErrorLog);
 ...
 * @private
 */
Tattle.prototype._buildEmailOptions = function(errorLog) {
    ...
    emailOptions.html = "";
    this._emailKeys.forEach(function(key) {

        if (errorLog[key]) {
            emailOptions.html += "<div><b>" + key + "</b>: " + errorLog[key] + "</div></br>";
        }
    });
    return emailOptions;
};
